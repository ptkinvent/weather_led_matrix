...
class WeatherClient():
    ...
    def query_weather(self):
        ...
        self.data = {
            'curr_temp': current_data['main']['temp'], # Deg F
            'high_temp': forecast_data['list'][0]['temp']['max'], # Deg F
            'low_temp': forecast_data['list'][0]['temp']['min'], # Deg F
            'description': current_data['weather'][0]['description'],
            'sunrise_time': current_data['sys']['sunrise'], # Datetime
            'sunset_time': current_data['sys']['sunset'], # Datetime
            'feels_like_temperature': current_data['main']['feels_like'], # Deg F
            'pressure': current_data['main']['pressure'], # Pa
            'humidity': current_data['main']['humidity'], # %
            'wind_speed': current_data['wind']['speed'], # mph
            'aqi': int(air_pollution_data['list'][0]['main']['aqi']), # 1-5
        }
